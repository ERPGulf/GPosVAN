...
export async function getCertificateDigestValue(certBase64: string): Promise<string> {
  ...
  const hashHexOutput = await Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256, pem, {
    encoding: Crypto.CryptoEncoding.HEX,
  });

  // Hash is derived, convert hex equivalent then base64
  const base64EncodedHash = Buffer.from(hashHexOutput, 'hex').toString('base64');
...
export function extractECPrivateKeyBytes(pemBase64: string): Uint8Array {
  ...
  const base64Key = pemContent.replace(/-----[^-]+-----/g, '').replace(/[\s\r\n]+/g, '');
